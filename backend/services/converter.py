...
def _convert_docx_to_png_list(docx_bytes: bytes) -> list:
    ...
    Falls back to text extraction if PDF conversion fails or docx2pdf unavailable.
    """
    
    # Try PDF conversion first if docx2pdf is available
    if HAS_DOCX2PDF:
        try:
        ...
        logging.info("docx2pdf not available, using text extraction fallback for DOCX")
    
    # Fallback: Extract text from DOCX and create image
    try:
        ...
        raise DocxConversionError(f"DOCX conversion failed (all methods): {str(fallback_error)}") from fallback_error
